...
try:
    ...
    logging.error(f"{time_now} LINE/Gemini SDK 載入失敗：{e}")
...
def init_line_bot():
    ...
    if not _line_handler and LINE_ACCESS_TOKEN and LINE_SECRET:
        try:
            ...
            app.logger.info(f"{time_now} LINE Bot 初始化成功")
        except Exception as e:
            app.logger.error(f"{time_now} LINE Bot 初始化失敗：{e}")
...
def callback():
    ...
    try:
        ...
        app.logger.error(f"{time_now} Invalid signature")
        abort(400)
    except Exception as e:
        app.logger.error(f"{time_now} Webhook error: {e}")
    return "OK"
...
def get_user_payment_methods(user_id):
    ...
    try:
        ...
        app.logger.error(f"{time_now} get_user_payment_methods 失敗: {e}")
    finally:
        conn.close()
    return [], []
...
if handler:
    ...
    def handle_follow(event):
        """處理使用者加入好友事件"""
        app.logger.info(f"{time_now} FollowEvent from {event.source.user_id}")
        ...
        with ApiClient(config) as api_client:
            ...
            try:
                ...
                app.logger.error(f"{time_now} handle_follow 錯誤: {e}")
