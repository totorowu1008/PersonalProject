...
def get_gemini_recommendation(line_user_id, reply_token, api: MessagingApi):
    ...
    - **本次消費類別**: 餐廳
    - **預估消費金額**: 新台幣 2500 元
    - **使用者目前擁有的支付工具**: 街口支付、台新GOGO卡、 玉山UBear卡
    ...
    try:
        ...
        #api.push_message(PushMessageRequest(to=line_user_id, messages=[TextMessage(type='text', quick_reply=None, sender=None, text='使用您現有的工具，推薦如下：', emojis=None, quote_token=None), TextMessage(type='text', quick_reply=None, sender=None, text='支付工具：街口支付，預估%數：2%，預估回饋金：50元', emojis=None, quote_token=None)]))
